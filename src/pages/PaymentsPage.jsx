...
export default function PaymentsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (from) { params.set('from', from); params.set('to', to); }

      const [paymentsData, summaryData] = await Promise.all([
        api(`/payments?${params}`),
        api(`/payments/summary?from=${from || ''}&to=${to || ''}`),
      ]);
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
          {[
            ...
            <button key={r.key} onClick={() => { setRange(r.key); setPagination(p => ({...p, page: 1})); }}
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          {summary.byMethod.slice(0, 2).map(m => {
            ...
            return (
              <SummaryCard key={m.method} label={method?.label || m.method}
                value={formatCOP(m.amount)} sub={`${m.count} pagos`} icon={method?.icon || '💰'} />
            );
          })}
        </div>
      )}
      ...
      {pagination.total > 30 && (
        <div className="flex justify-center gap-2 pt-2">
          ...
          <button disabled={pagination.page >= Math.ceil(pagination.total / 30)}
            onClick={() => setPagination(p => ({...p, page: p.page + 1}))}
