...
interface DashboardData {
  ...
  comparison: {
    ...
    appointment_change_pct: number | null;
  };
}
...
interface OperatorStat {
  ...
  first_name: string;
  last_name: string | null;
  total: number; // turnos asignados (sin filtrar por status)
  completed: number; // turnos entregados (status = 'delivered')
  revenue: number;
  avg_minutes: number | null;
}
...
const METHODS_LABEL: Record<string, string> = {
  cash: "Efectivo",
  nequi: "Nequi",
  daviplata: "Daviplata",
...
export default function ReportsPage() {
  ...
  const [operatorsData, setOperatorsData] = useState<OperatorsData | null>(
    null,
  );
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [dash, rev, svc, cust, ops] = await Promise.all([
        api<DashboardData>(`/reports/dashboard?period=${period}`),
        api<RevenueData>(`/reports/revenue?period=${period}`),
        api<ServicesData>(`/reports/services?period=${period}`),
        api<CustomersData>(`/reports/customers?period=${period}`),
        api<OperatorsData>(`/reports/operators?period=${period}`),
      ]);
      setDashboard(dash);
      setRevenue(rev);
      setServicesData(svc);
              ...
              <div className="flex items-end gap-1 h-40">
                {revenue.daily.map((d, i) => {
                  const maxRev = Math.max(
                    ...revenue.daily.map((x) => x.revenue),
                    1,
                  );
                  const pct = (d.revenue / maxRev) * 100;
                  const dateParsed = d.date.includes("T")
                    ? new Date(d.date)
                    : new Date(`${d.date}T12:00:00`);
                  const dayLabel = isNaN(dateParsed.getTime())
                    ? d.date
                    : dateParsed.toLocaleDateString("es-CO", {
                        weekday: "short",
                        day: "numeric",
                      });
                  return (
                      ...
                      <div key={i}>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="text-gray-500">
                            {s.count} turnos · {formatCOP(s.revenue)}
                          </span>
                        </div>
                  ...
                  {revenue.byMethod.map((m, i) => {
                    ...
                    return (
                      <div key={i}>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-gray-700 font-medium">
                            {METHODS_LABEL[m.method] ?? m.method}
                          </span>
                          <span className="text-gray-500">
                            {formatCOP(m.total)} ({m.count})
                          </span>
                        </div>
                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div
                  ...
                  {operatorsData.operators.map((op) => (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          {op.completed} entregados de {op.total} asignados
                          {op.avg_minutes
                            ? ` · ${op.avg_minutes} min prom.`
                            : ""}
                        </p>
                      </div>
                      <p className="font-semibold text-sm text-gray-900">
                        {formatCOP(op.revenue)}
                      </p>
                    </div>
                  ))}
                </div>
...
// Silenciar import no usado: PaymentMethod queda disponible si en el futuro
// se quiere endurecer el tipo de `m.method` en RevenueData.
