...
export default function AppointmentsPage() {
  ...
  const fetchAppointments = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api<{ data: Appointment[] }>(
        `/appointments?date=${selectedDate}&limit=100`,
      );
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      toast.success(
        newStatus === "cancelled" ? "Turno cancelado" : "Estado actualizado",
      );
      fetchAppointments();
    } catch (err) {
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <input
            title="Seleccionar fecha para ver los turnos programados"
            type="date"
            value={selectedDate}
      ...
      {loading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white rounded-xl p-4 h-28 animate-pulse"
            />
          ))}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          {a.operator_first_name && (
            <p className="text-xs text-gray-400">
              Operador: {a.operator_first_name}
            </p>
          )}
        </div>
